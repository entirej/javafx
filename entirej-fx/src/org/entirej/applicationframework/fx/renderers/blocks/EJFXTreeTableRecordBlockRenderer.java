...
import org.entirej.applicationframework.fx.renderers.blocks.def.EJFXTreeTableBlockDefinitionProperties;
...
import org.entirej.applicationframework.fx.utils.EJFXAbstractFilteredTreeTable;
...
public class EJFXTreeTableRecordBlockRenderer implements EJFXAppBlockRenderer
{
    ...
    private TreeTableView<EJDataRecord> _tableViewer;
    ...
    public Node createComponent()
    {
        ...
        _tableViewer = new TreeTableView<>(rootItem);
        Node tableNode = _tableViewer;
        final EJFXAbstractFilteredTreeTable filterTable;
        if (useFilter)
        {
            filterTable = new EJFXAbstractFilteredTreeTable(_tableViewer)
            {
        ...
        final String pid = rendererProp.getStringProperty(EJFXTreeTableBlockDefinitionProperties.PARENT_ITEM);
        final String rid = rendererProp.getStringProperty(EJFXTreeTableBlockDefinitionProperties.RELATION_ITEM);
        final String imageid = rendererProp.getStringProperty(EJFXTreeTableBlockDefinitionProperties.NODE_IMAGE_ITEM);
        expandLevel = rendererProp.getIntProperty(EJFXTreeTableBlockDefinitionProperties.NODE_EXPAND_LEVEL, -1);

        final Map<Object, Image> imageMap = new HashMap<>();

        _tableViewer.setShowRoot(false);

        if (imageid != null && _tableViewer.getColumns().size() > 0)
        {
            @SuppressWarnings("unchecked")
            TreeTableColumn<EJDataRecord, EJDataRecord> treeTableColumn = (TreeTableColumn<EJDataRecord, EJDataRecord>) _tableViewer.getColumns().get(0);
            @SuppressWarnings("unchecked")
            final TableColumn<EJDataRecord, EJDataRecord> labelProvider = (TableColumn<EJDataRecord, EJDataRecord>) treeTableColumn.getUserData();
            treeTableColumn.setCellFactory(new Callback<TreeTableColumn<EJDataRecord, EJDataRecord>, TreeTableCell<EJDataRecord, EJDataRecord>>()
            {

                @Override
                public TreeTableCell<EJDataRecord, EJDataRecord> call(TreeTableColumn<EJDataRecord, EJDataRecord> p)
                {

                    Callback<TableColumn<EJDataRecord, EJDataRecord>, TableCell<EJDataRecord, EJDataRecord>> cellFactory = labelProvider.getCellFactory();

                    final TableCell<EJDataRecord, EJDataRecord> cell = cellFactory.call(labelProvider);

                    TreeTableCell<EJDataRecord, EJDataRecord> treeTableCell = new TreeTableCell<EJDataRecord, EJDataRecord>()
                    {
                        @Override
                        protected void updateItem(EJDataRecord arg0, boolean empty)
                        {
                            if (cell instanceof VACell)
                            {
                                VACell vacCell = (VACell) cell;
                                vacCell.updateItem(arg0, empty);
                                setText(vacCell.getText());
                                setGraphic(vacCell.getGraphic());
                                getStyleClass().clear();
                                getStyleClass().addAll(vacCell.getStyleClass());
                                if (!empty)
                                {
                                    setGraphic(getImage(arg0));
                                }
                            }
                            else
                            {
                        ...
                        Node getImage(EJDataRecord record)
                        {
                            if (record == null)
                            {
                                return null;
        ...
        // _tableViewer.setCellFactory(new Callback<TreeView<EJDataRecord>,
        // TreeCell<EJDataRecord>>()
        // {
        //
        // @Override
        ...
        //
        // if
        // (!EJCoreVisualAttributeProperties.UNSPECIFIED.equals(baseVA.getBackgroundRGB()))
        ...
        // {
        // if
        // (!EJCoreVisualAttributeProperties.UNSPECIFIED.equals(attributes.getBackgroundRGB()))
        // {
        // getStyleClass().add(CSS_VA_CELL_BG);
        // }
        // }
    ...
    static class TreeNodeTextProvider
    {
        ...
        public TreeNodeTextProvider(TableColumn<EJDataRecord, ?> provider)
        {
    ...
    TreeNodeTextProvider createScreenItem(EJCoreMainScreenItemProperties itemProps)
    {
        ...
        if (renderer != null)
        {
            ...
            final TableColumn<EJDataRecord, EJDataRecord> labelProvider = itemRenderer.createColumnProvider(itemProps, item);
            ...
            if (labelProvider != null)
            {
                ...
                String labelOrientation = blockProperties.getStringProperty(EJFXTreeTableBlockDefinitionProperties.COLUMN_ALIGNMENT);
                ...
                int displayedWidth = blockProperties.getIntProperty(EJFXTreeTableBlockDefinitionProperties.DISPLAY_WIDTH_PROPERTY, 0);
                ...
                String visualAttribute = blockProperties.getStringProperty(EJFXTreeTableBlockDefinitionProperties.VISUAL_ATTRIBUTE_PROPERTY);
                ...
                tableCol.setCellFactory(new Callback<TreeTableColumn<EJDataRecord, EJDataRecord>, TreeTableCell<EJDataRecord, EJDataRecord>>()
                {
                    ...
                    public TreeTableCell<EJDataRecord, EJDataRecord> call(TreeTableColumn<EJDataRecord, EJDataRecord> p)
                    {
                        ...
                        {
                            @Override
                            protected void updateItem(EJDataRecord arg0, boolean arg1)
                            {
                                if (cell instanceof VACell)
                                {
                                    VACell vacCell = (VACell) cell;
                                    vacCell.updateItem(arg0, arg1);
                                    setText(vacCell.getText());
                                    setGraphic(vacCell.getGraphic());
                                    getStyleClass().clear();
                                    getStyleClass().addAll(vacCell.getStyleClass());
                ...
                tableCol.setCellValueFactory(new Callback<TreeTableColumn.CellDataFeatures<EJDataRecord, EJDataRecord>, ObservableValue<EJDataRecord>>()
                {

                    @Override
                    public ObservableValue<EJDataRecord> call(final javafx.scene.control.TreeTableColumn.CellDataFeatures<EJDataRecord, EJDataRecord> p)
                    {
                ...
                tableCol.setId(itemProps.getReferencedItemName());
                tableCol.setText(itemProps.getLabel());
                // labelProvider.setToolTipText(itemProps.getHint());

                tableCol.setResizable(blockProperties.getBooleanProperty(EJFXMultiRecordBlockDefinitionProperties.ALLOW_COLUMN_RESIZE, true));
                tableCol.setSortable(blockProperties.getBooleanProperty(EJFXMultiRecordBlockDefinitionProperties.ALLOW_ROW_SORTING, true));
                if (blockProperties.getBooleanProperty(EJFXMultiRecordBlockDefinitionProperties.ALLOW_ROW_SORTING, true))
                {
                    ...
                    if (columnSorter != null)
                        tableCol.setComparator(columnSorter);
                }
                ...
                return new TreeNodeTextProvider(labelProvider);
            }
